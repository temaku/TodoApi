...
func NewUserHandler(us user.UsersService, rs user.RoleService, ts rtoken.Service) *UserHandler {
...
func (uh *UserHandler) Authorized(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		role, errs := uh.roleService.Role(uint(claim.User.RoleID))
...
func (uh *UserHandler) SignUp(w http.ResponseWriter, r *http.Request) {
	...
	newUser, _ := uh.service.StoreUser(&u)
	if err != nil {
...
func (uh *UserHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	user, errs := uh.service.UserByUserName(u)
	if errs != nil {
		responses.ERROR(w, http.StatusUnauthorized, errors.New("authentication error"))
		return
	}

	if uh.checkAdmin(uint(user.RoleID)) == true{
		user.Password = ""
